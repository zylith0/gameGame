	...
	public void paintComponent(Graphics h) {
		...
		for (TPanel t : towerpanels) {
			...
			if (t.highlighted) {
				...
				g.fillOval(mouse.x - 200, mouse.y - 200, t.getRadius(), t.getRadius());
			}
		}
		// print towers
		for (Tower t : towers) {
				...
				for (Enemy em : enemies) {
					...
					if (distance <= t.getRadius()) {
						...
						if (ticks % t.firerate == 0) {
							...
							shots.add(new Projectile(t.getPoint().x, t.getPoint().y, (int) Math.round(dx),
									(int) Math.round(dy), 20, 20, theta));
						}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (e.getSource() instanceof Board && this.placing) {
			if (this.money >= towerpanels.get(toPlace).getCost()) {
				...
				/*
				 * \ 0-gun 1-machine gun 2-rocket launcher 3-lazzer 4-ice tower
				 */
				if (toPlace == 0) {
					towers.add(new Tower(e.getX() - 30, e.getY() - 30, towerpanels.get(toPlace).img, 200, 50));
				}
				if (toPlace == 1) {
					towers.add(new Tower(e.getX() - 30, e.getY() - 30, towerpanels.get(toPlace).img, 200, 100));
				}
				if (toPlace == 2) {
					towers.add(new Tower(e.getX() - 30, e.getY() - 30, towerpanels.get(toPlace).img, 400, 30));
				}
				if (toPlace == 3) {
					towers.add(new Tower(e.getX() - 30, e.getY() - 30, towerpanels.get(toPlace).img, 200, 50));
				}
